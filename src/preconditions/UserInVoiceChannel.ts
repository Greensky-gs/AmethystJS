...
export default new Precondition('UserInVoiceChannel')
    .setMessageRun(({ message }) => {
        if (!message.member?.voice?.channelId)
            return {
                ...
                metadata: {
                    code: commandDeniedCode.UserNotInVoiceChannel
                },
                type: 'message',
    ...
    .setButtonRun(({ button }) => {
        if (!(button.member as GuildMember)?.voice?.channelId)
            return {
                ok: false,
                message: 'Command executable only if user in voice channel',
                metadata: {
    ...
    .setModalRun(({ modal }) => {
        if (!(modal.member as GuildMember)?.voice?.channelId)
            return {
                ok: false,
                message: 'Command executable only if user in voice channel',
                metadata: {
