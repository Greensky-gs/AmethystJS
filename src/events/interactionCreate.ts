...
export default new AmethystEvent('interactionCreate', async (interaction) => {
    ...
    if (interaction.isAutocomplete()) {
        const listeners = interaction.client.autocompleteListeners.filter((x) => x.name === interaction.commandName || x.names.filter(y => y.commandName === interaction.commandName && ((y.optionName && y.optionName === interaction.options.getFocused(true).name)) || true));
