...
export class ControlPanel {
    ...
    private get components() {
        ...
        this.buttons.forEach((btn, i) => {
            ...
            const button = new ButtonBuilder().setCustomId(btn.id).setStyle(ButtonStyle[btn.style]);
            ...
            return {
                embeds: [
                    new EmbedBuilder()
                        ...
                        .setDescription(`This is the control panel of <@${this.client.application?.id}>`)
                        .setColor('#53CFF1')
                ],
    ...
    private async init() {
        this.channel = (this.client.channels.cache.get(this.channelId) ??
            (await this.client.channels.fetch(this.channelId).catch(log4js.trace))) as TextChannel;
        if (!this.channel) {
